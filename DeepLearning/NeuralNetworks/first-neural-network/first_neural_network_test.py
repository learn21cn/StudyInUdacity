...
data_path = './first-neural-network/Bike-Sharing-Dataset/hour.csv'
...
# 用于测试
batch = [1,2]
X12, y12 = train_features.iloc[batch].values, train_targets.iloc[batch]['cnt']
...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            # 激活函数 y = x  output_error_term = error   
            output_error_term = error
            
            # TODO: Calculate the hidden layer's contribution to the error                     
            hidden_error = np.dot(self.weights_hidden_to_output, output_error_term)
            
            # 对一维数组来说，转置还是行向量，参见上面的a，b测试
...
features_test, targets_test = train_features.iloc[batch][['temp','hum','windspeed']].values, train_targets.iloc[batch]['cnt']
print(features_test)
print(targets_test)
n_records = features_test.shape[0]
input_nodes = features_test.shape[1]
hidden_nodes = 2
output_nodes = 1
lr = 0.9

weights_input_to_hidden_test = np.random.normal(0.0, input_nodes**-0.5, (input_nodes, hidden_nodes))
weights_hidden_to_output_test = np.random.normal(0.0, hidden_nodes**-0.5, (hidden_nodes, output_nodes))

delta_weights_i_h_test = np.zeros(weights_input_to_hidden_test.shape)
delta_weights_h_o_test = np.zeros(weights_hidden_to_output_test.shape)

print('weights_input_to_hidden_test:')
print(weights_input_to_hidden_test)
print('weights_hidden_to_output_test:')
print(weights_hidden_to_output_test)
print('delta_weights_i_h_test:')
print(delta_weights_i_h_test)
print('delta_weights_h_o_test:')
print(delta_weights_h_o_test)
...
for X, y in zip(features_test, targets_test):
    ...
    hidden_inputs = np.dot(X, weights_input_to_hidden_test) 
    ...
    final_inputs = np.dot(hidden_outputs, weights_hidden_to_output_test)
    ...
    # TODO: Calculate the hidden layer's contribution to the error                     
    hidden_error = np.dot(weights_hidden_to_output_test, output_error_term)
    ...
    # Weight step (input to hidden)
    delta_weights_i_h_test += hidden_error_term * X[:, None]
    # Weight step (hidden to output)
    delta_weights_h_o_test += output_error_term * hidden_outputs[:, None]

    print("#####################################")

# TODO: Update the weights - Replace these values with your calculations.
# update hidden-to-output weights with gradient descent step
weights_hidden_to_output_test += lr * delta_weights_h_o_test / n_records
# update input-to-hidden weights with gradient descent step 
weights_input_to_hidden_test += lr * delta_weights_i_h_test / n_records 


# 利用调整后的系数计算
hidden_inputs_end = np.dot(features_test, weights_input_to_hidden_test) 
hidden_outputs_end = activation_function(hidden_inputs_end)   

print('hidden_inputs_end:')
print(hidden_inputs_end)
print('hidden_inputs_end:')
print(hidden_inputs_end)    
        
final_inputs_end = np.dot(hidden_outputs_end, weights_hidden_to_output_test) 
final_outputs_end = final_inputs_end   

print('final_outputs_end:')
print(final_outputs_end)       

y = final_outputs_end.T
Y = targets_test.values
# Y = np.array([targets_test.values])
